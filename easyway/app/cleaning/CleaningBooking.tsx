    ...
    >
      {['Licensed & insured', '4.9 stars · 3,000+ reviews', 'Vetted & background-checked', 'Same-day available'].map(t => (
        <div key={t} className="flex items-center gap-[7px] text-xs font-medium" style={{ color: 'rgba(255,255,255,0.7)' }}>
          <div className="w-[5px] h-[5px] rounded-full shrink-0" style={{ background: 'var(--ew-leaf)' }} />
          {t}
        </div>
      ))}
    </div>
...
function MobileOrderBar({ svcName, total }: { svcName: string; total: number }) {
  return (
    <div
      ...
      style={{ background: '#fafbfc', borderBottom: '1px solid rgba(51,63,54,0.08)' }}
    >
      <span className="text-[13px] font-semibold" style={{ color: 'var(--ew-forest)' }}>{svcName}</span>
      <span className="text-base font-extrabold" style={{ color: 'var(--ew-leaf)' }}>{total > 0 ? `$${total}` : '—'}</span>
    </div>
  )
}
...
function StepContent({ step, st, mobile, bookedSlots, onIncr, onDecr, onSelectOne, onUpd }: {
  ...
  if (step.type === 'counter') return (
    <>
      {step.sub && <p className="text-[13px] mb-4 leading-relaxed" style={{ color: 'rgba(51,63,54,0.6)' }}>{step.sub}</p>}
      <div className="rounded-md overflow-hidden" style={{ border: '1px solid rgba(51,63,54,0.08)' }}>
        {items.map((item, i) => {
          ...
          return (
            <div
              ...
              style={{
                borderBottom: i < items.length - 1 ? '1px solid rgba(51,63,54,0.08)' : 'none',
            ...
            >
              <div className="flex-1 pr-3 min-w-0">
                <div
                  ...
                  style={{ color: qty > 0 ? 'var(--ew-forest)' : 'rgba(51,63,54,0.7)', fontWeight: qty > 0 ? 600 : 400 }}
                >
                  {item.label}
                </div>
                {item.desc && <div className="text-[11px] mt-0.5" style={{ color: '#9aa5b4' }}>{item.desc}</div>}
                {item.price > 0 && <div className="text-[11px] mt-0.5" style={{ color: 'var(--ew-leaf)' }}>${item.price} per item</div>}
              </div>
              <div className="flex items-center gap-2.5 shrink-0">
                {qty > 0 && (
                  <button
                    ...
                    style={{ border: '1.5px solid rgba(51,63,54,0.15)', color: 'rgba(51,63,54,0.6)' }}
                  >
                    <Minus size={12} />
                  </button>
                )}
                {qty > 0 && (
                  <span className="text-sm font-bold min-w-[16px] text-center" style={{ color: 'var(--ew-forest)' }}>
                    {qty}
                  </span>
                )}
                <button
                  ...
                  style={{
                    ...
                    background: qty > 0 ? 'rgba(75,118,22,0.1)' : 'var(--ew-bg)',
                    color: qty > 0 ? 'var(--ew-forest)' : 'rgba(51,63,54,0.6)',
                  }}
                >
                  <Plus size={12} />
                </button>
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1 pr-2">
                  <div
                    className={`${mobile ? 'text-[13px]' : 'text-sm'}`}
                    style={{ color: sel ? 'var(--ew-forest)' : 'rgba(51,63,54,0.7)', fontWeight: sel ? 600 : 400 }}
                  >
                    {item.label}
                  </div>
                  {item.desc && <div className="text-[11px] mt-[3px] leading-snug" style={{ color: '#9aa5b4' }}>{item.desc}</div>}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {item.price > 0 && (
                    <span className="text-[13px] font-semibold" style={{ color: 'var(--ew-leaf)' }}>+${item.price}</span>
                  )}
                  {sel && (
                    <div
  ...
  if (step.type === 'textarea') {
    ...
    return (
      <>
        {step.sub && <p className="text-[13px] mb-4 leading-relaxed" style={{ color: 'rgba(51,63,54,0.6)' }}>{step.sub}</p>}
        <div className="relative">
          <FileText size={15} className="absolute left-3 top-3 pointer-events-none" style={{ color: '#9aa5b4' }} />
          <textarea
  ...
  if (step.type === 'photo') return (
    <>
      <p className="text-[13px] mb-4 leading-relaxed" style={{ color: 'rgba(51,63,54,0.6)' }}>
      ...
      <div
        ...
        onMouseEnter={(e) => (e.currentTarget.style.borderColor = 'var(--ew-sky)')}
        onMouseLeave={(e) => (e.currentTarget.style.borderColor = 'rgba(51,63,54,0.15)')}
      >
        <Upload size={26} strokeWidth={1.5} className="mx-auto mb-2.5" style={{ color: 'var(--ew-leaf)' }} />
        <p className="text-sm m-0 mb-1" style={{ color: 'rgba(51,63,54,0.7)' }}>Click to upload or drag & drop</p>
        <p className="text-xs m-0" style={{ color: '#9aa5b4' }}>PNG, JPG, HEIC up to 20MB each</p>
      </div>
    </>
  )
      ...
      <>
        ...
        <div className={`flex gap-[7px] mb-4 ${mobile ? 'overflow-x-auto pb-1' : ''}`}>
          {days.map(d => {
            ...
            const dayBookedCount = times.filter(t => bookedSlots.includes(`${d.date}__${t}`)).length
            ...
            return (
              <button
                ...
                style={{
                  border: `1.5px solid ${
                    fullyBooked ? 'rgba(51,63,54,0.08)' :
                    active ? 'var(--ew-leaf)' : 'rgba(51,63,54,0.15)'
                  }`,
                  background: fullyBooked ? '#fafbfc' :
                    active ? 'rgba(75,118,22,0.08)' : 'var(--ew-bg)',
                  opacity: fullyBooked ? 0.4 : 1,
                  cursor: fullyBooked ? 'not-allowed' : 'pointer',
                }}
              >
                <span className="text-[10px] font-medium" style={{ color: '#9aa5b4' }}>{d.short}</span>
                <span
                  className="text-[15px] font-bold mt-0.5"
                  style={{ color: active ? 'var(--ew-forest)' : 'rgba(51,63,54,0.85)' }}
                >
                  {d.num}
                </span>
                <span className="text-[10px] mt-px" style={{ color: '#9aa5b4' }}>{d.mon}</span>
              </button>
            )
          })}
        </div>
            ...
            {times.map(t => {
              ...
              return (
                <button
                  ...
                  style={{
                    border: `1.5px solid ${
                      booked ? 'rgba(51,63,54,0.08)' :
                      active ? 'var(--ew-leaf)' : 'rgba(51,63,54,0.15)'
                    }`,
                    background: booked ? '#fafbfc' :
                      active ? 'rgba(75,118,22,0.08)' : 'var(--ew-bg)',
                    color: booked ? '#c4c9d4' :
                      active ? 'var(--ew-forest)' : 'rgba(51,63,54,0.7)',
        ...
        {!st.date && (
          <p className="text-xs mt-2" style={{ color: '#9aa5b4' }}>Select a date to see available times.</p>
        )}
      </>
    )
  }
    ...
    <>
      ...
      <div className="flex flex-col gap-3.5">
        ...
        <label className="flex items-start gap-[9px] cursor-pointer">
          ...
          />
          <span className="text-[13px] leading-relaxed" style={{ color: 'rgba(51,63,54,0.7)' }}>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            if (!st.loading) (e.currentTarget as HTMLElement).style.background = 'var(--ew-sky)'
          }}
          onMouseLeave={(e) => {
    ...
    >
      ...
      >
        ...
        <h1
          ...
          style={{ color: '#ffffff' }}
        >
          Clean Home,{' '}<em className="italic" style={{ color: 'var(--ew-sky)' }}>Effortlessly.</em>
        </h1>
        <p className="mt-3.5 text-sm max-w-[460px] leading-[1.7]" style={{ color: 'rgba(255,255,255,0.6)' }}>
      ...
      <div className={`max-w-[1000px] mx-auto ${mobile ? 'px-4 pt-7 pb-16' : 'px-6 pt-12 pb-20'}`}>
        <p className="text-[11px] tracking-[0.18em] uppercase font-semibold mb-2" style={{ color: '#9aa5b4' }}>
          Select Service
        </p>
        <p className="text-[13px] mb-6 leading-relaxed" style={{ color: 'rgba(51,63,54,0.6)' }}>
          Most services include add-ons and can be combined if no single option meets your needs.
        </p>

        {services.length === 0 ? (
          <div className="text-center py-16 text-sm" style={{ color: '#9aa5b4' }}>No services available yet.</div>
        ) : (
          <div
          ...
          >
            {services.map(svc => (
              <div
                ...
                onMouseEnter={(e) => (e.currentTarget.style.borderColor = 'var(--ew-sky)')}
                onMouseLeave={(e) => (e.currentTarget.style.borderColor = 'rgba(51,63,54,0.12)')}
              >
                <div
                  ...
                  style={{
                    ...
                    background: '#edf0f4',
                    backgroundImage: svc.image_url ? `url('${svc.image_url}')` : undefined,
                  }}
                >
                  <div
                    className="absolute inset-0"
                    style={{ background: 'linear-gradient(to top, rgba(75,118,22,0.13), transparent)' }}
                  />
                </div>
                <div className={mobile ? 'p-3 pb-3.5' : 'px-[18px] pt-4 pb-5'}>
                  ...
                  {!mobile && (
                    <p className="text-xs leading-relaxed mb-3" style={{ color: 'rgba(51,63,54,0.6)' }}>
                      {svc.description}
                    </p>
                  )}
                  <div
        ...
        <div className={`max-w-[540px] mx-auto px-4 ${mobile ? 'mt-8' : 'mt-14'}`}>
          ...
          >
            ...
            <div className={`text-center ${mobile ? 'px-5 py-8' : 'px-9 py-12'}`}>
              ...
              <p className="text-sm leading-[1.7] mb-2" style={{ color: 'rgba(51,63,54,0.6)' }}>
                Your <strong style={{ color: 'var(--ew-forest)' }}>{st.service?.name}</strong> appointment has been submitted.
              </p>
              <p className="text-sm leading-[1.7] mb-6" style={{ color: 'rgba(51,63,54,0.6)' }}>
                We'll confirm details to <strong style={{ color: 'var(--ew-forest)' }}>{st.email || 'your email'}</strong> shortly.
              </p>
              {lines.length > 0 && (
                <div
                  className="rounded-md px-[18px] py-3.5 mb-6 text-left"
                  style={{ background: '#fafbfc', border: '1px solid rgba(51,63,54,0.08)' }}
                >
                  {lines.map(l => (
                    <div key={l.key} className="flex justify-between text-[13px] py-[3px]" style={{ color: 'rgba(51,63,54,0.7)' }}>
                  ...
                  <div
                    ...
                    style={{ borderTop: '1px solid rgba(51,63,54,0.08)', color: 'var(--ew-forest)' }}
                  >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="w-px h-[18px] shrink-0" style={{ background: 'rgba(255,255,255,0.15)' }} />
      ...
      <div className={`max-w-[900px] mx-auto ${mobile ? 'px-0 pt-4 pb-16' : 'px-6 pt-8 pb-20'}`}>
        <div
          ...
          style={{
            background: 'var(--ew-bg)',
            border: mobile ? '1px solid rgba(51,63,54,0.12)' : '1px solid rgba(51,63,54,0.12)',
        ...
        >
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            >
              <h2 className={`font-bold m-0 truncate text-white ${mobile ? 'text-[13px]' : 'text-[15px]'}`}>
                {stepTitle}
              </h2>
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  <div
                    ...
                    style={{ width: `${progress}%`, background: 'var(--ew-leaf)' }}
                  />
                </div>
              </div>
            </div>
            ...
            {!isContact && (
              <div className={mobile ? 'px-4 pb-4' : 'px-6 pb-6'}>
                <button
                  ...
                  style={{ background: 'var(--ew-forest)', color: '#ffffff' }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--ew-sky)')}
                  onMouseLeave={(e) => (e.currentTarget.style.background = 'var(--ew-forest)')}
                >
          ...
          {!mobile && (
            <div
              ...
              style={{ background: '#fafbfc', borderLeft: '1px solid rgba(51,63,54,0.08)' }}
            >
              <p className="text-[11px] font-bold tracking-[0.08em] uppercase m-0" style={{ color: 'var(--ew-forest)' }}>
                {st.service.name}
              </p>
              {lines.length > 0 ? (
                <div className="flex flex-col gap-[5px]">
                  {lines.map(l => (
                    <div key={l.key} className="text-xs" style={{ color: 'rgba(51,63,54,0.7)' }}>
                      {l.qty}x {l.label}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs italic m-0" style={{ color: '#9aa5b4' }}>No items selected yet</p>
              )}
              {st.date && (
                <div className="text-xs">
                  <div className="font-semibold" style={{ color: 'var(--ew-leaf)' }}>
                    {new Date(st.date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
                  </div>
                  {st.time && <div className="mt-0.5" style={{ color: 'rgba(51,63,54,0.7)' }}>{st.time}</div>}
                </div>
              )}
              <div
              ...
              >
                <span className="text-xs" style={{ color: '#9aa5b4' }}>Total</span>
                <span className="text-xl font-extrabold" style={{ color: 'var(--ew-leaf)' }}>
                  {total > 0 ? `$${total}` : '—'}
                </span>
              </div>
            </div>
        ...
        <div className={`grid gap-2.5 ${mobile ? 'grid-cols-1 px-4 pt-4' : 'grid-cols-3 mt-5'}`}>
          {[
            { icon: <ShieldCheck size={18} strokeWidth={1.8} />, title: 'Licensed & insured', sub: 'All staff background-checked and certified' },
            { icon: <Star size={18} strokeWidth={1.8} />, title: '4.9 star rating', sub: 'Over 3,000 verified customer reviews' },
            { icon: <Clock size={18} strokeWidth={1.8} />, title: 'Same-day available', sub: 'Fast response for urgent cleaning needs' },
          ].map(({ icon, title, sub }) => (
            <div
              key={title}
            ...
            >
              ...
              <div>
                <p className="text-[13px] font-bold mb-0.5" style={{ color: 'var(--ew-forest)' }}>{title}</p>
                <p className="text-xs leading-relaxed m-0" style={{ color: 'rgba(51,63,54,0.55)' }}>{sub}</p>
              </div>
            </div>
          ))}
        </div>

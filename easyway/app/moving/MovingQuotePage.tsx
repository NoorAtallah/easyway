...
import {
  ...
  Loader2, AlertCircle, Plus, Minus, ShoppingCart, X,
} from "lucide-react";
...
function ItemCard({ item, count, onInc, onDec }: {
  item: MovingItem; count: number; onInc: () => void; onDec: () => void;
}) {
  return (
    <div
    ...
    >
      <div className="relative h-[130px]" style={{ background: "rgba(51,63,54,0.04)" }}>
        ...
        {count > 0 && (
          <div
            className="absolute top-2 right-2 text-xs font-bold w-[22px] h-[22px] rounded-full flex items-center justify-center"
      ...
      <div className="p-3.5">
        <p className="m-0 mb-0.5 text-sm font-medium" style={{ color: "var(--ew-forest)" }}>{item.name}</p>
        <p className="m-0 mb-3 text-xs" style={{ color: "#9aa5b4" }}>{item.cuft} cu.ft</p>
...
export default function MovingQuotePage({ items, pricingRanges }: { items: MovingItem[]; pricingRanges: PricingRange[] }) {
  ...
  const handleZipChange = useCallback((e: React.ChangeEvent<HTMLInputElement>, which: "pickup" | "dropoff") => {
    ...
    if (!isValidZip(val)) { setter({ info: null, status: "idle", error: "" }); setDistanceMiles(null); return; }
    ...
    zipTimers.current[which] = setTimeout(async () => {
      try {
        ...
        if (isValidZip(p) && isValidZip(d)) {
          ...
          try { const dist = await fetchDistance(p, d); setDistanceMiles(dist); }
  ...
  const estimatedBase = matchedRange && totalCuft > 0 ? Math.round(totalCuft * Number(matchedRange.price_per_cuft)) : null;
        ...
        >
          ...
          {referenceId && (
            <p
              ...
              style={{ color: "var(--ew-sky)", background: "rgba(27,110,180,0.08)" }}
            >
              {referenceId}
            </p>
          )}
  ...
  return (
    ...
    >
            ...
            <div key={n} className="flex items-center gap-2">
              <div
                ...
                style={{
                  ...
                  color: step === n ? "#ffffff" : step > n ? "#ffffff" : "#9aa5b4",
                }}
              >
              ...
              <span
                ...
                style={{ color: step === n ? "var(--ew-forest)" : "#9aa5b4" }}
              >
                {label}
              </span>
              ...
              <div className="p-5 sm:p-7 flex flex-col gap-4">
                ...
                {(distanceLoading || distanceMiles !== null) && (
                  <div
                    ...
                    style={{
                      color: "var(--ew-sky)",
                      background: "rgba(27,110,180,0.06)",
                      border: "1px solid rgba(27,110,180,0.18)",
                    }}
                  >
                    {distanceLoading
          ...
          <div>
            <div className="flex items-start justify-between mb-6 gap-4">
              <div>
                ...
                >
                  What are you{" "}
                  <em className="italic" style={{ color: "var(--ew-sky)" }}>taking?</em>
                </h2>
                {distanceMiles !== null && (
              ...
              >
                ...
                {totalItems > 0 ? `${totalItems} item${totalItems !== 1 ? "s" : ""} · ${totalCuft} cu.ft` : "Your move list"}
              </button>
            </div>
            ...
            {sections.length > 0 && (
              <div
                ...
                style={{ borderBottom: "1px solid rgba(51,63,54,0.12)" }}
              >
                {sections.map(s => {
                  const sectionCount = s.items.reduce((a, item) => a + (counts[item.id] || 0), 0);
                  const active = s.name === activeSection;
                  return (
                    <button
                      key={s.name}
                      onClick={() => setActiveSection(s.name)}
                      className="flex items-center gap-[7px] px-[18px] py-2.5 -mb-px bg-transparent border-none border-b-2 whitespace-nowrap text-[13px] font-['DM_Sans',sans-serif] cursor-pointer transition-[color,border-color] duration-150"
                      style={{
                        borderBottom: active ? "2px solid var(--ew-sky)" : "2px solid transparent",
                        color: active ? "var(--ew-forest)" : "#9aa5b4",
                    ...
                    >
                      ...
                      {sectionCount > 0 && (
                        <span
                          className="text-[11px] font-medium rounded-full px-[7px] py-px"
            ...
            {currentSection && (
              <div className="grid gap-3.5 mb-8" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))" }}>
                {currentSection.items.map(item => (
                  <ItemCard key={item.id} item={item} count={counts[item.id] || 0} onInc={() => inc(item.id)} onDec={() => dec(item.id)} />
                ))}
              </div>
            )}
            ...
            <button
              ...
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "var(--ew-forest)"}
            >
              {submitting ? <><Loader2 size={15} className="animate-spin" /> Submitting…</> : <>Request Quote <ArrowRight size={15} /></>}
            </button>
            {totalItems === 0 && (
      ...
      {estimatedRange !== null && (
        <div
          className="flex justify-between pt-2.5 mx-4 sm:mx-auto max-w-[760px]"
          style={{ borderTop: "1px solid rgba(51,63,54,0.08)" }}
        >
          <span className="text-xs font-semibold" style={{ color: "var(--ew-forest)" }}>Estimated price</span>
          <span className="text-[15px] font-extrabold" style={{ color: "var(--ew-forest)" }}>
        ...
        <>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2
                className="font-['Playfair_Display',Georgia,serif] text-[1.3rem] font-bold m-0"
                style={{ color: "var(--ew-forest)" }}
              >
                Your move list
              </h2>
            ...
            {totalItems === 0 ? (
              ...
              <>
                  ...
                  {items.filter(item => (counts[item.id] || 0) > 0).map(item => {
                    ...
                    return (
                      <div
                        ...
                        style={{ background: "rgba(51,63,54,0.04)" }}
                      >
                        {item.image_url && (
                          <img src={item.image_url} alt={item.name} className="w-[42px] h-[42px] rounded-md object-cover shrink-0" />
                        )}
                        ...
                        <div className="flex items-center gap-[5px]">
                          <button
                            onClick={() => dec(item.id)}
                            className="w-[22px] h-[22px] rounded cursor-pointer flex items-center justify-center"
                            style={{ border: "1px solid rgba(51,63,54,0.15)", background: "var(--ew-bg)", color: "rgba(51,63,54,0.6)" }}
                          >
                            <Minus size={10} />
                          </button>
                          ...
                          <button
                            onClick={() => inc(item.id)}
                            className="w-[22px] h-[22px] rounded cursor-pointer flex items-center justify-center"
                            style={{ border: "1px solid var(--ew-sky)", background: "rgba(27,110,180,0.08)", color: "var(--ew-forest)" }}
                          >
                            <Plus size={10} />
                          </button>
                        </div>
                ...
                <div className="mt-4 p-4 rounded-xl" style={{ background: "rgba(51,63,54,0.04)" }}>
                  ...
                  {estimatedRange !== null && (
                    <div
                      className="mt-4 rounded-lg px-5 py-4 flex items-center justify-between gap-3"
                      style={{
                        background: "rgba(27,110,180,0.06)",
                        border: "1px solid rgba(27,110,180,0.18)",
                      }}
                    >
                      ...
                      <span
                        className="text-[1.4rem] font-extrabold font-['Playfair_Display',serif] shrink-0"
                        style={{ color: "var(--ew-forest)" }}
                      >
                        ${estimatedRange.low.toLocaleString()} – ${estimatedRange.high.toLocaleString()}
                      </span>

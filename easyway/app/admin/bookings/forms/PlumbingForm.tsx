...
export function PlumbingForm({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = () => {
    if (!firstName || !lastName || !email || !phone || !address || !city || !zip || !jobDescription.trim()) {
      setError('Please fill in all required fields including the job description.')
      return
    }
    setError(null)
    startTransition(async () => {
      const result = await createAdminBooking('plumbing', {
  ...
  return (
    <div className="p-6 flex flex-col gap-5 overflow-y-auto max-h-[70vh]">
      ...
      <div>
        <p className="text-[11px] tracking-[0.1em] uppercase text-[#718096] font-bold mb-1.5">Job description *</p>
        <textarea
          value={jobDescription} onChange={e => setJobDescription(e.target.value)} rows={4}

...
import { updateLandscapingQuoteStatus, getQuoteStatusHistory } from './actions'
import { EmailModal } from './EmailModal'
import type { LandscapingQuote as Quote } from '@/types/landscaping'
...
function StatusDropdown({
  ...
  const handleSelect = (status: Status) => {
    ...
    startTransition(async () => {
      const result = await updateLandscapingQuoteStatus(quoteId, status)
...
function DetailPanel({
  ...
  useEffect(() => {
    ...
    getQuoteStatusHistory(quote.id, 'landscaping').then(setHistory)
    ...
    <>
      ...
      <aside className="fixed top-0 right-0 h-screen w-full max-w-[500px] bg-white z-[201] shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-200">
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <section>
            <p className="text-[11px] tracking-[0.12em] uppercase text-[#718096] font-bold mb-2">Job details</p>
            <div className="text-sm text-[#4a5568]">
              Job size: <span className="font-semibold text-[#1a2e35]">{quote.job_size_label}</span>
            </div>
          </section>
                  ...
                  {history.map(h => (
                    <div key={h.id} className="border-l-2 border-[#8cc7c4] pl-3 py-0.5">
                      ...
                      <p className="text-xs text-[#4a5568] m-0">
                        by <span className="font-semibold text-[#1a2e35]">{h.changed_by_name || h.changed_by_email || 'Unknown'}</span>
                      </p>
...
// ---------- Main table ----------
export default function LandscapingQuotesTable({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('landscaping_quotes_realtime')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'landscaping_quotes',
        },
        (payload) => {
          ...
          if (payload.eventType === 'DELETE') {
            const removed = payload.old as Pick<Quote, 'id'>
    ...
    return (
      <div className="bg-white border border-[#dde3ea] rounded-lg p-12 text-center">
        <p className="text-[#718096] text-sm">
          No landscaping quotes yet. They&apos;ll appear here when customers submit the form.
        </p>
      </div>
    )
  }
...
<div className="md:hidden divide-y divide-[#edf0f4]">
  {quotes.map(q => (
    <div
      ...
      className="p-4 hover:bg-[#fafbfc] cursor-pointer"
    >
  ...
  {q.reference_id && (
    <span className="ml-2 font-mono font-semibold text-[#8cc7c4]">{q.reference_id}</span>
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        <div>
          <div className="font-semibold text-[#1a2e35] text-sm">
  ...
  <Mail size={11} /> {q.email}
        ...
        <a href={`tel:${q.phone}`} onClick={e => e.stopPropagation()}
          className="flex items-center gap-1.5 hover:text-[#8cc7c4]">
  ...
  <table className="w-full text-sm">
    <thead className="bg-[#fafbfc] border-b border-[#dde3ea]">
      <tr className="text-left text-[11px] tracking-[0.1em] uppercase text-[#4a5568] font-bold">
        ...
        <th className="px-4 py-3">Job size</th>
  ...
  <Mail size={12} /> {q.email}
          ...
          <td className="px-4 py-3">
            <div className="flex items-start gap-1.5 text-[#4a5568] text-xs">
              ...
              <div>
                <div>{q.address}</div>
                <div className="text-[#9aa5b4]">{q.city}, {q.zip_code}</div>
              </div>
            </div>
          </td>
          <td className="px-4 py-3">
            <span className="text-[#4a5568]">{q.job_size_label}</span>
          </td>
          <td className="px-4 py-3">
            <StatusDropdown
              quoteId={q.id}
      ...
      {toast && <Toast message={toast} onDone={() => setToast(null)} />}
        ...
        <EmailModal
          ...
          jobDetails={emailQuote.job_size_label}

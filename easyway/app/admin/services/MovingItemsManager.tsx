...
function ImageUpload({
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("moving-items").getPublicUrl(path);
...
function AddItemModal({
  ...
  const handleSubmit = () => {
    ...
    if (!form.cuft || isNaN(Number(form.cuft)))
      return setError("Valid cu.ft is required");
      ...
      <div className="fixed inset-0 z-[201] flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-[400px] overflow-hidden">
          <div className="bg-[#1a2e35] px-6 py-4 flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="border-none bg-transparent text-white/60 hover:text-white cursor-pointer p-0"
            >
              <X size={18} />
            </button>
          </div>
          <div className="p-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                value={form.cuft}
                onChange={(e) =>
                  setForm((p) => ({ ...p, cuft: e.target.value }))
                }
                placeholder="e.g. 70"
                className="w-full py-[10px] px-3 bg-[#fafbfc] border-[1.5px] border-[#dde3ea] rounded text-sm text-[#1a202c] outline-none focus:border-[#8cc7c4]"
              />
...
// ── Main component ────────────────────────────────────────────
export default function MovingItemsManager({
  items: initialItems,
}: {
  items: MovingItem[];
}) {
    ...
    <div className="p-4 md:p-8 font-['DM_Sans',sans-serif]">
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[#718096] mt-1">
            Manage items shown in the moving calculator. Click any name or
            volume to edit, then save.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-4">
        {sections.map((section) => {
          ...
          const sortedItems = [...sectionItems].sort(
            (a, b) => a.sort_order - b.sort_order,
          );
          ...
          return (
            <div
              key={section}
              className="bg-white border border-[#dde3ea] rounded-xl overflow-hidden"
            >
              {/* Section header */}
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-['Playfair_Display',serif] font-bold text-[#1a2e35]">
                    {section}
                  </span>
                  <span className="text-xs text-[#9aa5b4]">
                    {activeCount}/{sectionItems.length} active
                  </span>
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-0.5">
                            <InlineEdit
                              ...
                              isDirty={
                                isDirty &&
                                "name" in (pendingChanges[item.id] ?? {})
                              }
                              className="text-sm font-medium text-[#1a2e35]"
                            />
                            <span className="text-xs text-[#9aa5b4] md:hidden">
                              {item.cuft} cu.ft
                            </span>
                          </div>
                          ...
                          <div className="hidden md:block">
                            <InlineEdit
                              ...
                              isDirty={
                                isDirty &&
                                "cuft" in (pendingChanges[item.id] ?? {})
                              }
                          ...
                          <div className="hidden md:block">
                            <span
                              className={`text-[10px] font-bold uppercase tracking-[0.5px] px-2 py-[3px] rounded-full border ${
                                item.is_active
              ...
              {saveSuccess ? (
                ...
                <>
                  ...
                  <span className="text-white/70">
                    ...
                    item{Object.keys(pendingChanges).length !== 1 ? "s" : ""}{" "}

...
import {
  createPlumbingField, updatePlumbingField, deletePlumbingField,
  createPlumbingFieldOption, updatePlumbingFieldOption, deletePlumbingFieldOption,
} from './plumbing-actions'
import type { PlumbingField, PlumbingFieldType, PlumbingFieldOption } from '@/types/plumbing'

const FIELD_TYPES: { value: PlumbingFieldType; label: string }[] = [
...
const ICON_OPTIONS = [
  'FileText','MapPin','Building2','Home','Wrench','User','Mail','Phone',
  'Calendar','Clock','DollarSign','Hash','Info','MessageSquare',
]

type FieldChange = Partial<{
  key: string; label: string; placeholder: string; help_text: string
  type: PlumbingFieldType; icon: string; required: boolean
...
function AddFieldModal({
  ...
  onClose: () => void
  onAdded: (field: PlumbingField) => void
  nextSortOrder: number
}) {
  const [form, setForm] = useState<{
    key: string; label: string; type: PlumbingFieldType
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await createPlumbingField({
        key: form.key.trim(),
          ...
          <div className="p-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="e.g. When did the problem start?"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  value={form.type}
                  onChange={e => setForm(p => ({ ...p, type: e.target.value as PlumbingFieldType }))}
...
function OptionsEditor({
  ...
  field: PlumbingField
  onChange: (next: PlumbingFieldOption[]) => void
}) {
  ...
  const handleAdd = () => {
    ...
    const value = draft.value.trim() || draft.label.toLowerCase().replace(/\s+/g, '_')
    startTransition(async () => {
      const result = await createPlumbingFieldOption({
        field_id: field.id,
        label: draft.label.trim(),
        value,
        sort_order: field.plumbing_field_options.length,
      })
      if (result.success) {
        onChange([
          ...field.plumbing_field_options,
          {
            id: result.id, field_id: field.id,
  ...
  const handleDelete = (id: string) => {
    startTransition(async () => {
      await deletePlumbingFieldOption(id)
      onChange(field.plumbing_field_options.filter(o => o.id !== id))
    })
  }
  ...
  const handleUpdate = (id: string, label: string) => {
    ...
    startTransition(async () => {
      await updatePlumbingFieldOption(id, { label, value })
      onChange(field.plumbing_field_options.map(o =>
  ...
  return (
    <div className="ml-9 mt-2 mb-2 pl-4 border-l-2 border-[#edf0f4] flex flex-col gap-1.5">
      ...
      {field.plumbing_field_options.length === 0 && !adding && (
      ...
      {field.plumbing_field_options
        ...
        .map(opt => (
          <div key={opt.id} className="flex items-center gap-2 text-sm">
            ...
            <InlineEdit
              value={opt.label}
              onSave={v => handleUpdate(opt.id, v)}
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onKeyDown={e => {
              ...
              if (e.key === 'Escape') { setAdding(false); setDraft({ label: '', value: '' }) }
            }}
            onBlur={() => { if (!draft.label) setAdding(false) }}
          ...
          <button
            onClick={handleAdd}
            disabled={isPending}
            className="text-xs text-[#8cc7c4] hover:text-[#6fb8b4] bg-transparent border-none cursor-pointer disabled:opacity-50"
          >
            Add
          </button>
        </div>
...
// ── Main component ───────────────────────────────────────────
export default function PlumbingFieldsManager({ fields: initialFields }: { fields: PlumbingField[] }) {
    ...
    try {
      await Promise.all(
        Object.entries(pendingChanges).map(([id, changes]) =>
          updatePlumbingField(id, changes)
        )
      )
      setPendingChanges({})
      setSaveSuccess(true)
  ...
  const handleDelete = (id: string, label: string) => {
    ...
    startTransition(async () => { await deletePlumbingField(id) })
  }

  const handleAddField = (field: PlumbingField) => {
    setFields(prev => [...prev, field])
  }

  const handleUpdateOptions = (id: string, options: PlumbingFieldOption[]) => {
    setFields(prev => prev.map(f =>
      f.id === id ? { ...f, plumbing_field_options: options } : f
    ))
  }
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          <h1 className="font-['Playfair_Display',serif] text-2xl font-bold text-[#1a2e35] m-0">
            Plumbing Form Fields
          </h1>
          <p className="text-sm text-[#718096] mt-1">
            Add, edit, reorder, and remove the questions shown on the plumbing quote form.
          </p>
        </div>
        <button
        ...
        {sortedFields.length === 0 ? (
          ...
          <div className="divide-y divide-[#edf0f4]">
            {sortedFields.map((field, i) => {
              ...
              const canExpand = field.type === 'select' || true // always allow detail edit
                ...
                <div key={field.id}>
                  ...
                  >
                    ...
                    <div className="hidden md:block">
                      <select
                        value={field.type}
                        onChange={e => queueChange(field.id, { type: e.target.value as PlumbingFieldType })}
                  ...
                  {isExpanded && (
                    <div className="px-5 py-3 bg-[#fafbfc] border-t border-[#edf0f4] flex flex-col gap-3">
                      ...
                      {field.type === 'select' && (
                        <OptionsEditor
                          field={field}
                          onChange={opts => handleUpdateOptions(field.id, opts)}
                        />
                      )}
                    </div>
                  )}
              ...
              {saveSuccess ? (
                ...
                <>
                  ...
                  <span className="text-white/70">
                    ...
                    field{Object.keys(pendingChanges).length !== 1 ? 's' : ''} with unsaved changes
                  </span>
                </>
              )}
            </div>

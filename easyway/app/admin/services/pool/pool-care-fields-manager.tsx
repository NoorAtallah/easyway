...
import {
  createPoolCareField, updatePoolCareField, deletePoolCareField,
  createPoolCareFieldOption, updatePoolCareFieldOption, deletePoolCareFieldOption,
} from './pool-actions'
...
function AddFieldModal({
  ...
  onClose: () => void
  onAdded: (field: PoolCareField) => void
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await createPoolCareField({
        key: form.key.trim(),
          ...
          <div className="p-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="e.g. Pool condition"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="e.g. pool_condition"
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={onClose} className="flex-1 py-2.5 border border-[#dde3ea] rounded-lg text-sm text-[#718096] cursor-pointer bg-white hover:bg-[#fafbfc]">
                Cancel
              </button>
              <button
...
function OptionsEditor({
  ...
  const visibleSaved = field.pool_care_field_options.filter(o => !pendingDeletedOptionIds.has(o.id))
  const newOptions = pendingNewOptions.filter(o => o.field_id === field.id)
  const totalSortOrder = visibleSaved.length + newOptions.length
...
// ── Main component ───────────────────────────────────────────
export default function PoolCareFieldsManager({ fields: initialFields }: { fields: PoolCareField[] }) {
  const safeInitial = (Array.isArray(initialFields) ? initialFields : []).map(f => ({
    ...f,
    pool_care_field_options: Array.isArray(f.pool_care_field_options) ? f.pool_care_field_options : [],
  }))
  ...
  const queueOptionChange = (id: string, change: OptionChange) => {
    ...
    setFields(prev => prev.map(f => ({
      ...f,
      pool_care_field_options: f.pool_care_field_options.map(o => o.id === id ? { ...o, ...change } : o),
    })))
  }
  ...
  const handleSaveAll = async () => {
    ...
    setSaving(true)
    try {
      await Promise.all(Object.entries(pendingChanges).map(([id, changes]) => updatePoolCareField(id, changes)))
      await Promise.all(Object.entries(pendingOptionChanges).map(([id, changes]) => updatePoolCareFieldOption(id, changes)))
      await Promise.all(Array.from(pendingDeletedOptionIds).map(id => deletePoolCareFieldOption(id)))

      const createResults = await Promise.all(
        pendingNewOptions.map(opt =>
          createPoolCareFieldOption({ field_id: opt.field_id, label: opt.label, value: opt.value, sort_order: opt.sort_order })
            .then(r => ({ ...r, opt }))
        )
      )

      setFields(prev => prev.map(f => {
        const remaining = f.pool_care_field_options.filter(o => !pendingDeletedOptionIds.has(o.id))
        ...
        return { ...f, pool_care_field_options: [...remaining, ...created] }
      }))
  ...
  const handleDelete = (id: string, label: string) => {
    ...
    startTransition(async () => { await deletePoolCareField(id) })
  }
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h2 className="font-['Playfair_Display',serif] text-xl font-bold text-[#1a2e35] m-0">
            Pool Care Form Fields
          </h2>
          <p className="text-sm text-[#718096] mt-1">
            Manage the fields shown on the pool care quote form.
          </p>
        </div>
        <button
                ...
                <div key={field.id}>
                  <div className={`grid grid-cols-[1fr_auto] md:grid-cols-[40px_1fr_120px_100px_90px_100px] gap-3 items-center px-5 py-3 transition-colors ${!field.is_active ? 'opacity-50' : ''} ${isDirty ? 'bg-amber-50/40' : ''}`}>
                    ...
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => queueChange(field.id, { is_active: !field.is_active })} title={field.is_active ? 'Hide' : 'Show'} className="p-1.5 text-[#718096] hover:text-[#1a2e35] bg-[#f4f6f8] hover:bg-[#edf0f4] rounded-md border-none cursor-pointer">
                        {field.is_active ? <EyeOff size={13} /> : <Eye size={13} />}
                      </button>
                      <button onClick={() => handleDelete(field.id, field.label)} title="Delete" className="p-1.5 text-red-400 hover:text-red-600 bg-[#f4f6f8] hover:bg-red-50 rounded-md border-none cursor-pointer">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
